...
class ObjectSerializer
{
    ...
    public static function isEnumClass($class): bool
    {
        if ((!is_string($class) && !is_object($class))) {
            return false;
        }
        return method_exists($class, 'getAllowableEnumValues')
            && method_exists($class, 'getValue');
    }

    public static function isFlagEnum($type): bool
    {
        if ((!is_string($type) && !is_object($type))) {
            return false;
        }
        return self::isEnumClass($type) && method_exists($type, 'isFlags') && $type::isFlags();
    }
    ...
    public static function sanitizeForSerialization($data, $type = null, $format = null)
    {
        ...
        if (is_array($data)) {
            ...
            // If it's a Flag Enum sent as an array, convert it to a string
            if (self::isFlagEnum($type)) {
                return implode(',', $data);
            }
            return $data;
        }
    ...
    public static function deserialize($data, $class, $httpHeaders = null)
    {
        ...
        if (strcasecmp(substr($class, -2), '[]') === 0) {
            ...
            // If it's an array Enum and data is not an array, return the data
            if (self::isEnumClass($subClass) && !is_array($data) && !is_object($data)) {
                return $data;
            }
        ...
        if (method_exists($class, 'getAllowableEnumValues') && method_exists($class, 'validateValue')) {
            if (!$class::validateValue($data)) {
            ...
            if (!empty($discriminator) && isset($data->{$discriminator}) && is_string($data->{$discriminator})) {
                ...
                if (!isset($mappings[$data->{$discriminator}])) {
                    throw new \InvalidArgumentException("Discriminator '{$discriminator}' value '{$data->{$discriminator}}' for class '$class' not found in mappings");
                }
                $class = $mappings[$data->{$discriminator}];
            }
